...
ok(!service.includes('applyGroupUmrahFilters') && service.includes('groupUmrahFieldAuthority') && service.includes("['status','workflow_status','booking_status']"), 'Group Umrah filters use one deterministic field authority');
...
ok(service.includes("$maps=['status'=>['status','workflow_status','booking_status']") && service.includes("'customer_name','customer','party_name','customer_id','party_id','client_id','customer_party_id'"), 'Group Umrah filter and display authority resolves deterministic primary aliases');
ok(service.includes("$authority=$this->groupUmrahFieldAuthority($filter,$cols)") && service.includes("$authority['source']==='group'") && service.includes("whereExists(fn($x)=>$x->select(DB::raw(1))->from('bookings as b')"), 'Group Umrah query selects schema authority and Booking fallback deterministically');
ok(service.includes("$authority=function(string $field)use(&$booking,$b,$id,$groupColumns)") && service.includes("$selected=$this->groupUmrahFieldAuthority($field,$groupColumns)") && service.includes("$booking??=$this->bookingRecord($id)"), 'Group Umrah row display reuses the same authority resolver');
for (const field of ['status','customer','branch','agent','salesperson']) ok(service.includes(`$authority('${field}')`) || service.includes(`groupUmrahFieldAuthority('${field}'`), `Group Umrah ${field} filter/display parity is explicit`);
ok(!service.includes("if($key==='group-umrah')$column=match") && service.includes("$basis=$filters['date_type']??'booking'") && service.includes('groupUmrahQuery'), 'Group Umrah has one date authority path');
ok(service.includes("'branch_name','branch','branch_id','office_id'") && service.includes("'agent_name','agent','agent_id'") && service.includes("'salesperson_name','salesperson','salesperson_id'"), 'Group Umrah branch agent and salesperson authority is adaptive');
