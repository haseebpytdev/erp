...
ok(version === 'v1.1.33.324-ERP11.3.324', 'ERP-11.3.316 packaged release metadata is current');
...
ok(productView.includes('et-dedicated-product-name') && productView.includes('{{ $productLabel }} Workspace') && !productView.includes('<h2>{{ $productLabel }}</h2>'), 'dedicated product header keeps the workspace label without a duplicate runtime-card heading');
ok(productView.includes('data-etgp-dedicated-product-host') && productView.includes('data-etgp-dedicated-product-body'), 'dedicated product runtime host and body remain present');
