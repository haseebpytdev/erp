...
ok(migration.includes("$table->string('passport_no'") && migration.includes("$table->string('first_name'") && migration.includes("$table->unsignedBigInteger('created_by')->nullable()"), 'canonical fields and nullable creator exist');
...
ok(migration.includes('where(function ($query)') && migration.includes('orWhereDate'), 'backfill duplicate priority supports passport and name+DOB');
