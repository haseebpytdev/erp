...
ok(navigation.includes('parent.replaceChild(mountedRoot,target)') && navigation.includes('target=launcher.closest(\'main\')') && navigation.includes('transaction={parent:parent,original:target,mountedRoot:mountedRoot}'), 'only the proven native main workspace target is replaced with rollback state');
