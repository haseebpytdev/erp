...
ok(finalizer.includes("heading.style.setProperty('padding', '0', 'important')"), 'section heading does not rely on padding to protect its first row');
...
ok(finalizer.includes("if (index === 0) row.style.setProperty('margin-top', '9px', 'important')"), 'first permitted row owns explicit child separation so active state cannot overlap heading');
