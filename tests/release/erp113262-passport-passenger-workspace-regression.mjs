...
ok(progressive.includes('etgpBookingCommercialDisplay113302') && progressive.includes('persisted_booking_value') && progressive.includes('bookingValue:commercial.bookingValue'), 'persisted booking value feeds Final Booking Value while product totals remain breakdown data');
...
ok(progressive.includes("var amount=Number(data.final_booking_value)") && progressive.includes("if(!Number.isFinite(amount))amount=Math.max(0,Number(data.booking_value||0))") && progressive.includes("etgpAirSetKpi113124('Booking Value'"), 'top Booking Value KPI uses resolved final authority with operational booking fallback');
ok(progressive.includes('var passengerCountAuthority=Number(data.passenger_count)') && progressive.includes("etgpAirSetKpi113124('Passengers',String(Math.floor(passengerCountAuthority))"), 'main Booking passenger KPI consumes server passenger_count');
ok(progressive.includes('var ticketCountAuthority=Number(data.air_ticket_count)') && progressive.includes('etgpAirUpdateTicketMetric113106(Math.floor(ticketCountAuthority))'), 'main Booking Tickets KPI consumes Air summary ticket authority');
const refreshAuthoritySlice = progressive.slice(progressive.indexOf('var etgpRefreshPersistedBookingState113153='), progressive.indexOf('window.etgpRefreshPersistedBookingState113153='));
ok(!refreshAuthoritySlice.includes('etgp-current-passenger-table') && refreshAuthoritySlice.indexOf('var passengerCountAuthority=Number(data.passenger_count)') < refreshAuthoritySlice.indexOf("etgpAirSetKpi113124('Passengers'"), 'server KPI refresh does not derive passenger authority from duplicated DOM rows');
ok(progressive.includes('var etgpTicketKpiState113126') && progressive.includes('new MutationObserver'), 'Tickets single-writer MutationObserver guard remains intact');
ok(!progressive.includes("Number(commercial.bookingValue||0)"), 'top Booking Value KPI does not use persisted commercial value');
ok(progressive.includes("steps[0].classList.add('is-current')") && progressive.includes("step.classList.remove('is-done')") && progressive.includes("step.classList.remove('is-current')"), 'Booking & Passengers remains current while Products and Review stay inactive');
ok(operationalSummary.includes("'booking_value' => $summary['gross_customer_total']") && operationalSummary.includes("'persisted_booking_value' => $summary['persisted_booking_value']") && operationalSummary.includes("'supplier_cost' => $summary['persisted_supplier_cost']"), 'operational summary preserves its booking value contract and exposes persisted commercial authorities separately');
...
const commercialDisplay = commercialContext.window.etgpBookingCommercialDisplay113302({ product_customer_totals: { air: 100, hotel: 50 }, final_booking_value: 999, supplier_cost_total: 400, gross_margin: 599 });
ok(commercialDisplay.products.air === 100 && commercialDisplay.products.hotel === 50 && commercialDisplay.bookingValue === 999 && commercialDisplay.supplierCost === 400 && commercialDisplay.margin === 599, 'actual commercial authority runtime separates product breakdown, resolved final booking value, supplier cost, and display margin');
const populatedCommercialDisplay = commercialContext.window.etgpBookingCommercialDisplay113302({ product_customer_totals: { air: 100, hotel: 50 }, final_booking_value: 999, persisted_booking_value: 888, supplier_cost_total: 400, supplier_cost: 350, gross_margin: 599 });
ok(populatedCommercialDisplay.bookingValue === 999 && populatedCommercialDisplay.supplierCost === 400 && populatedCommercialDisplay.margin === 599, 'actual commercial authority runtime prefers resolved commercial fields');
