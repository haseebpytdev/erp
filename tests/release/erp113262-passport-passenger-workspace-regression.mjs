...
ok(progressive.includes("var amount=Math.max(0,Number(data.booking_value||0))") && progressive.includes("etgpAirSetKpi113124('Booking Value'"), 'top Booking Value KPI uses the operational data.booking_value authority');
...
ok(operationalSummary.includes("'booking_value' => array_sum($totals)") && operationalSummary.includes("'persisted_booking_value' => $bookingValue") && operationalSummary.includes("'supplier_cost' => $supplierCost"), 'operational summary preserves its booking value contract and exposes persisted commercial authorities separately');
...
ok(commercialDisplay.products.air === 100 && commercialDisplay.products.hotel === 50 && commercialDisplay.bookingValue === null && commercialDisplay.supplierCost === 400 && commercialDisplay.margin === null, 'actual commercial authority runtime rejects missing persisted booking value without inventing a total');
const populatedCommercialDisplay = commercialContext.window.etgpBookingCommercialDisplay113302({ product_customer_totals: { air: 100, hotel: 50 }, persisted_booking_value: 999, supplier_cost: 400 });
ok(populatedCommercialDisplay.bookingValue === 999 && populatedCommercialDisplay.supplierCost === 400 && populatedCommercialDisplay.margin === 599, 'actual commercial authority runtime separates product breakdown, persisted booking value, supplier cost, and display margin');
const zeroCommercialDisplay = commercialContext.window.etgpBookingCommercialDisplay113302({ persisted_booking_value: 0, supplier_cost: 0 });
