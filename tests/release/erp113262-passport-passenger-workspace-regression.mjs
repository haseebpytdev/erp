...
const mrzDigit = text => { const map = c => c === '<' ? 0 : /[0-9]/.test(c) ? Number(c) : c.charCodeAt(0) - 55; return String([...text].reduce((sum, c, i) => sum + map(c) * [7, 3, 1][i % 3], 0) % 10); };
const pakLine1 = 'P<PAKKHAN<<ALI<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.padEnd(44, '<');
const pakPassport = 'A1234567<';
const pakDob = '900101';
const pakExpiry = '300101';
const pakLine2Base = `${pakPassport}${mrzDigit(pakPassport)}PAK${pakDob}${mrzDigit(pakDob)}M${pakExpiry}${mrzDigit(pakExpiry)}<<<<<<<<<<<<<<`.slice(0, 42) + mrzDigit(`${pakPassport}${mrzDigit(pakPassport)}PAK${pakDob}${mrzDigit(pakDob)}M${pakExpiry}${mrzDigit(pakExpiry)}<<<<<<<<<<<<<<`.slice(28, 42));
const pakComposite = `${pakLine2Base.slice(0, 10)}${pakLine2Base.slice(13, 20)}${pakLine2Base.slice(21, 43)}`;
const pakFixture = `${pakLine1}\n${pakLine2Base}${mrzDigit(pakComposite)}`;
...
ok(['corrected', 'ambiguous'].includes(context.window.ETPassportMRZ.parse(pakFixture.replace('900101', '9O0101')).correctionState) || context.window.ETPassportMRZ.parse(pakFixture.replace('900101', '9O0101')).review === true, 'O to zero correction is position-aware');
ok(['corrected', 'ambiguous'].includes(context.window.ETPassportMRZ.parse(pakFixture.replace('900101', '9I0101')).correctionState) || context.window.ETPassportMRZ.parse(pakFixture.replace('900101', '9I0101')).review === true, 'I to one correction is position-aware');
ok(context.window.ETPassportMRZ.parse(pakFixture.replace('KHAN', 'K0AN')).surname.includes('0'), 'letter-zone digit confusion is not silently auto-corrected');
