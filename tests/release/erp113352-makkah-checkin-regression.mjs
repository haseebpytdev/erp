...
ok(service.includes("destination_code','to','destination"), 'destination alias fallback');
  ok(service.includes('$fromCode=$this->movementEventCode'), 'movement sector canonical source');
  ok(!service.includes('$event->from_code=$this->movementEventCode'), 'route normalization side-effect free');
  ok(!service.includes("where('event.'.$from"), 'single alias origin prefilter removed');
  ok(!service.includes("where('event.'.$to"), 'single alias destination prefilter removed');
  ok(service.includes("===strtoupper(trim((string)$filters['origin']))"), 'exact origin filter');
  ok(service.includes("===strtoupper(trim((string)$filters['destination']))"), 'exact destination filter');
  ok(service.includes("!==''?$fromCode.'-'.$toCode:'—'"), 'unresolved/partial sector fallback');
