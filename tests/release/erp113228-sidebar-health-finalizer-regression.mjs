...
ok(!finalizer.includes("heading.style.setProperty('margin'"), 'finalizer no longer writes sidebar heading margin inline');
ok(!finalizer.includes("row.style.setProperty('margin-top'"), 'finalizer no longer writes first-row spacing inline');
ok(shellCss.includes('margin:16px 8px 8px!important'), 'shell CSS owns final section spacing');
