...
function setup(mode='fast',opts={}){
  const d=new D();d.meta=new E('meta');d.meta.setAttribute('name','csrf-token');d.meta.content='csrf';const html=d.documentElement,body=d.body,head=d.head,main=new E('main');const launcher=new E('section');launcher.setAttribute('data-et-booking-products-launcher','1');const airLink=new E('a');airLink.href='/operations/bookings/31/products/air';airLink.setAttribute('href',airLink.href);airLink.textContent='Air';launcher.appendChild(airLink);const register=new E('a');register.href='/operations/bookings';register.setAttribute('href',register.href);register.textContent='Booking Register';main.appendChild(launcher);main.appendChild(register);body.appendChild(main);const side=new E('aside');side.className='sidebar';side.textContent='Dashboard Easy Ticket Administration';body.appendChild(side);const requests=[];let dynamicCore=false,dynamicAir=false,assignCount=0;const loc={origin:'https://erp.test',href:'https://erp.test/operations/bookings/31',pathname:'/operations/bookings/31',assign:()=>{assignCount++;}};const win={location:loc,history:{state:null,pushState:(_,__,u)=>{loc.href='https://erp.test'+u;loc.pathname=u;}},confirm:()=>true,requestAnimationFrame:f=>f(),addEventListener:()=>{},etBookingFocus:{mountPresentation:()=>opts.failPresentation?false:true}};
  ...
  if(mode==='direct'){win.location.href='https://erp.test/operations/bookings/31/products/air';win.location.pathname='/operations/bookings/31/products/air';body.appendChild(fragmentRoot());vm.runInContext(coreSource,ctx);vm.runInContext(airSource,ctx);} else {vm.runInContext(navSource,ctx);}
  return {ctx,win,d,body,head,main,airLink,requests,get assignCount(){return assignCount;},get core(){return win.etDedicatedProductCore;},get air(){return win.etDedicatedAirProduct;},dynamicCore:()=>dynamicCore,dynamicAir:()=>dynamicAir};
}
...
let h=await runFast(); assert.equal(h.requests.filter(x=>x.url.includes('/fragment')).length,1);assert.equal(h.requests.filter(x=>x.url.endsWith('/air-product')).length,1);assert.equal(h.requests.find(x=>x.url.endsWith('/air-product')).method,'GET');const oldBookingMain=h.main;const root=h.d.querySelector('[data-etgp-dedicated-product="1"]');assert.ok(root);assert.equal(h.core.getBookingId(),31);assert.equal(h.core.getProductKey(),'air');assert.equal(root.getAttribute('data-etgp-air-mounted'),'1');assert.equal(oldBookingMain.isConnected,false);assert.notEqual(root.parentNode,oldBookingMain);assert.notEqual(root.parentNode&&root.parentNode.tagName,'MAIN');const text=root.textContent;for(const v of ['FASTPNR31','AIRPNR31','SA-731','LHE','JED','FAST ALI','FAST SARA','TKT-501','TKT-502','30 KG','240664'])assert.ok(text.includes(v)||root.querySelectorAll('input').some(i=>String(i.value).includes(v)),v+' rendered');
