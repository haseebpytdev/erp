...
ok(sidebar.includes("'TRAVEL REPORTS' => ['travel reports']"), 'compact Travel Reports section');
ok(sidebar.includes('nav-section') && sidebar.includes('nav-item') && sidebar.includes('et-ui-nav-icon'), 'native Reports section classes');
ok(sidebar.includes("contains(concat(\" \", normalize-space(@class), \" \"), \" nav \")") && sidebar.includes('foreach ($container->childNodes as $child)'), 'live nav direct-child targeting');
...
ok(sidebar.includes('matchingTagEnd') && sidebar.includes('sidebarPattern'), 'raw fragment replacement is sidebar scoped');
ok(sidebar.includes('(?:^|\\s)nav(?:\\s|$)') && sidebar.includes('count($matches[0]) !== 1'), 'raw nav class token and uniqueness guard');
