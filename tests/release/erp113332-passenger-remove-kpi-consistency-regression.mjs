...
ok(removeController.includes('reconcileAirServiceSnapshots($booking, $affectedAirServiceIds)'), 'only captured Air service IDs reach reconciliation');
ok(removeController.includes('whereIn(\'id\', $affectedServiceIds)'), 'reconciliation is scoped to affected services only');
ok(removeController.includes('if (! $affectedServiceIds'), 'no-Air-row removal skips all service snapshot writes');
ok(removeController.includes('booking_services') && removeController.includes("where('booking_id', $booking)"), 'service reconciliation remains booking-scoped');
...
ok(removeController.includes('selling_total') && removeController.includes('customer_sale_amount') && removeController.includes('receivable_amount'), 'Air customer authority aliases match normal Air save');
ok(removeController.includes('net_supplier_cost') && removeController.includes('supplier_cost_amount') && removeController.includes('purchase_price'), 'Air supplier authority aliases match normal Air save');
ok(removeController.includes("['unit_price', 'sale_price', 'selling_price']") && removeController.includes('customer / $count'), 'Air snapshot quantity and unit-price contract is preserved');
