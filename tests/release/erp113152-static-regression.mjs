...
contains(hierarchy, "if(k!==family)return", 'child navigation is context-specific');
contains(hierarchy, 'findNav=()', 'Travel Masters navigation is scoped');
contains(hierarchy, 'cloneNode(true)', 'child links are cloned');
contains(hierarchy, 'airlineTabs', 'Airlines family exact-tab detection');
contains(hierarchy, 'data-et-travel-master-airlines', 'Airlines child runtime hook');
...
contains(hierarchy, "const unwanted=['Vehicle Types'", 'unwanted primary links are explicitly hidden');
