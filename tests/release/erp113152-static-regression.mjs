...
contains(hierarchy, "label==='Visa Management'?'/master-data/travel-masters/visa-management?tab=rates'", 'Visa Management deterministic creation');
...
contains(hierarchy, 'Saudi Visa Companies','Visa child order');
contains(hierarchy, "['Airlines','Flight Routes']", 'Airlines child order');
contains(hierarchy, 'if(!nativeStrip)return', 'child navigation is context-specific');
contains(hierarchy, 'nativeStrip', 'Travel Masters navigation is scoped');
contains(hierarchy, 'textNorm', 'text normalizer');
contains(hierarchy, 'controlNorm', 'control normalizer');
contains(hierarchy, 'normalizedLabels=new Set(labels.map(textNorm))', 'normalized label set');
contains(hierarchy, 'normalizedLabels.has(textNorm(anchor.textContent))', 'normalized anchor comparison');
contains(hierarchy, "airlineTabs=[tabs.airlines,tabs.airlineCodes]", 'Airlines family exact-tab detection');
excludes(hierarchy, 'data-et-travel-master-airlines="113281"', 'second Airlines authority removed');
contains(hierarchy, "/master-data/travel-masters/flight-routes", 'Flight Routes child href');
contains(hierarchy, "['Airlines','Flight Routes']", 'Airlines child order');
excludes(hierarchy.match(/labels=\[([^\]]+)\]/)?.[1] || '', 'Flight Routes', 'Flight Routes absent from seven top-level labels');
contains(hierarchy, "const primary=document.createElement('nav')", 'primary presentation strip created');
