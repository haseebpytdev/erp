...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        @media(max-width:1150px){.audit-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:1050px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.steps{grid-template-columns:1fr}}@media(max-width:650px){.grid,.audit-grid{grid-template-columns:1fr}.topbar{align-items:flex-start;flex-direction:column}}
    </style>
</head>
<body>
<div class="page">
    <div class="topbar">
        <div>
            <div class="kicker">Fresh Production Start · {{ config('et_erp_release.release', 'ERP') }} Execution Safety Preview</div>
            <h1>Day-Zero Database Reset</h1>
            <div class="sub">Live-schema safety audit for the approved fresh-production plan. Foreign keys, child-before-parent delete order and counter restart columns are inspected while destructive execution remains locked.</div>
        </div>
    ...
    <div class="notice warn">
        <strong>No delete can run from this safety-preview build.</strong> Zero REVIEW tables is necessary but not sufficient. Foreign-key blockers, dependency cycles and unrecognized counter restart columns also block any future execution release.
    </div>
    ...
    <div class="card">
        <div class="card-head"><div class="card-title">Execution safety audit</div><div class="card-note">Read-only runtime inspection. This section does not delete, update, truncate or reset any table.</div></div>
        <div class="card-body">
            <div class="audit-grid">
                ...
                <div class="audit"><div class="audit-label">FK blockers</div><div class="audit-value {{ (int)$plan['fk_blockers'] === 0 ? 'good' : 'bad' }}">{{ number_format((int)$plan['fk_blockers']) }}</div></div>
            ...
            @if(!empty($plan['fk_blocker_items']))
                <div class="notice" style="margin:12px 0 0"><strong>Foreign-key blockers found.</strong> @foreach($plan['fk_blocker_items'] as $blocker)<div class="small"><strong>{{ $blocker['child_table'] }}.{{ $blocker['child_column'] }}</strong> → {{ $blocker['parent_table'] }}.{{ $blocker['parent_column'] }} · {{ $blocker['reason'] }}</div>@endforeach</div>
            @endif

            @if(!empty($plan['dependency_cycle_tables']))
                <div class="notice" style="margin:12px 0 0"><strong>Dependency cycle detected.</strong> <span class="small">{{ implode(', ', $plan['dependency_cycle_tables']) }}</span></div>
            @endif
        ...
        <div class="card-body">
            <div class="steps">
                ...
                <div class="step">
                    ...
                    <div class="step-text">A later authorization release may enable execution only after zero REVIEW tables, zero FK blockers, no dependency cycles, a complete delete order, a valid counter reset plan and a fresh full backup.</div>
                    <div class="locked"><strong>Execution enabled:</strong> NO<br><strong>Safety preview:</strong> {{ $plan['safety_preview_ready'] ? 'PASS' : 'BLOCKED' }}<br><strong>Required phrase:</strong> {{ $plan['confirmation'] }}<br><strong>Current REVIEW tables:</strong> {{ number_format((int)$plan['review_tables']) }}<br><strong>FK blockers:</strong> {{ number_format((int)$plan['fk_blockers']) }}<br><strong>Dependency cycles:</strong> {{ number_format((int)$plan['dependency_cycles']) }}</div>
