...
<div class="page">
    <div class="topbar">
        ...
        <h1>Restart Production Document Numbers From 1</h1>
    ...
    <div class="grid">
        ...
        <div class="metric"><div class="metric-label">CLEAR tables empty</div><div class="metric-value {{ (int)$plan['empty_clear_tables'] === (int)$plan['clear_tables'] ? 'good' : 'bad' }}">{{ number_format((int)$plan['empty_clear_tables']) }} / {{ number_format((int)$plan['clear_tables']) }}</div></div>
        <div class="metric"><div class="metric-label">Identity targets</div><div class="metric-value">{{ number_format((int)$plan['identity_target_count']) }}</div><div class="metric-sub">Will restart next inserted ID at 1</div></div>
    ...
    <div class="card">
        <div class="card-head"><div class="card-title">Identity / auto-increment targets</div><div class="card-note">Only empty Day-Zero CLEAR tables with database-managed identities are included.</div></div>
        ...
        @forelse($plan['identity_targets'] as $target)
            <tr><td><strong>{{ $target['table'] }}</strong></td><td>{{ $target['column'] }}</td><td>{{ $target['driver'] }}</td><td><span class="tag">1</span></td></tr>
        @empty
            ...
            @elseif($plan['ready'])
                <div class="formbox">
                    <form method="POST" action="{{ route('system.production-data-reset.execute') }}" onsubmit="return confirm('FINAL CHECK: Reset approved Day-One document identities and counters so new numbering starts from 1?');">
                        ...
                        <button class="btn" type="submit">FINALIZE DAY-ONE NUMBERS FROM 1</button>
                    </form>
                </div>
            @else
