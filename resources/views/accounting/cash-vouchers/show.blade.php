...
<div class="cvs27" data-et-cash-voucher-show="{{ config('et_erp_release.release', 'ERP-11.3') }}">
  ...
  <section class="cvs27-card">
    ...
    <div class="cvs27-body">
      <div class="cvs27-grid">
        ...
        @if(!in_array($row->voucher_type,['expense','contra'],true))
        ...
        <div class="cvs27-item"><b>{{ $row->voucher_type==='contra' ? 'Transfer Method' : 'Payment Method' }}</b>{{ $row->payment_method }}</div>
        <div class="cvs27-item"><b>{{ $row->voucher_type==='contra' ? 'From Cash / Bank Account' : 'Cash / Bank Account' }}</b>@if(!empty($accountLedgerUrls[$row->cash_bank_account_code]))<a href="{{ $accountLedgerUrls[$row->cash_bank_account_code] }}">{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}</a>@else{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}@endif</div>
        @if($row->voucher_type==='contra' && $contraDetail)
          <div class="cvs27-item"><b>To Cash / Bank Account</b>@if(!empty($accountLedgerUrls[$contraDetail->destination_account_code]))<a href="{{ $accountLedgerUrls[$contraDetail->destination_account_code] }}">{{ $contraDetail->destination_account_code }} · {{ $contraDetail->destination_account_name }}</a>@else{{ $contraDetail->destination_account_code }} · {{ $contraDetail->destination_account_name }}@endif</div>
        @endif
        @if($row->voucher_type!=='contra')<div class="cvs27-item"><b>Bank Name</b>{{ $row->bank_name ?: '—' }}</div>@endif
        ...
        <div class="cvs27-item"><b>Posting Reference</b>@if($row->posting_reference && $journalUrl)<a href="{{ $journalUrl }}">{{ $row->posting_reference }}</a>@else{{ $row->posting_reference ?: '—' }}@endif</div>
      ...
      <table class="cvs27-table">
        ...
        <tbody>
          @foreach($expenseLines as $line)
            <tr><td>{{ $line->line_no }}</td><td><strong>@if(!empty($accountLedgerUrls[$line->expense_account_code]))<a href="{{ $accountLedgerUrls[$line->expense_account_code] }}">{{ $line->expense_account_code }} · {{ $line->expense_account_name }}</a>@else{{ $line->expense_account_code }} · {{ $line->expense_account_name }}@endif</strong></td><td>{{ $line->description ?: '—' }}</td><td>{{ $line->currency_code }} {{ number_format($line->amount,2) }}</td><td>PKR {{ number_format($line->base_amount,2) }}</td></tr>
          @endforeach
        </tbody>
  ...
  @if($row->voucher_type==='contra' && $contraDetail && $postings->isEmpty())
    <section class="cvs27-card">
      ...
      <div class="cvs27-post">
        ...
        <div>@if(!empty($accountLedgerUrls[$contraDetail->destination_account_code]))<a href="{{ $accountLedgerUrls[$contraDetail->destination_account_code] }}">{{ $contraDetail->destination_account_code }} · {{ $contraDetail->destination_account_name }}</a>@else{{ $contraDetail->destination_account_code }} · {{ $contraDetail->destination_account_name }}@endif</div><div>{{ $row->narration ?: 'Contra transfer received' }}</div><div style="text-align:right">{{ number_format($contraDetail->base_amount,2) }}</div><div style="text-align:right">0.00</div>
        <div>@if(!empty($accountLedgerUrls[$row->cash_bank_account_code]))<a href="{{ $accountLedgerUrls[$row->cash_bank_account_code] }}">{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}</a>@else{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}@endif</div><div>{{ $row->narration ?: 'Contra transfer sent' }}</div><div style="text-align:right">0.00</div><div style="text-align:right">{{ number_format($contraDetail->base_amount,2) }}</div>
      </div>
    </section>
  @elseif($row->voucher_type==='expense' && $postings->isEmpty())
    <section class="cvs27-card">
      <div class="cvs27-card-head">Accounting Preview</div>
      <div class="cvs27-post">
        <div><b>Account</b></div><div><b>Description</b></div><div style="text-align:right"><b>Debit (Base)</b></div><div style="text-align:right"><b>Credit (Base)</b></div>
        @foreach($expenseLines as $line)
          <div>@if(!empty($accountLedgerUrls[$line->expense_account_code]))<a href="{{ $accountLedgerUrls[$line->expense_account_code] }}">{{ $line->expense_account_code }} · {{ $line->expense_account_name }}</a>@else{{ $line->expense_account_code }} · {{ $line->expense_account_name }}@endif</div><div>{{ $line->description ?: $row->narration }}</div><div style="text-align:right">{{ number_format($line->base_amount,2) }}</div><div style="text-align:right">0.00</div>
        @endforeach
        <div>@if(!empty($accountLedgerUrls[$row->cash_bank_account_code]))<a href="{{ $accountLedgerUrls[$row->cash_bank_account_code] }}">{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}</a>@else{{ $row->cash_bank_account_code }} · {{ $row->cash_bank_account_name }}@endif</div><div>{{ $row->narration ?: 'Direct expense payment' }}</div><div style="text-align:right">0.00</div><div style="text-align:right">{{ number_format($expenseLines->sum('base_amount'),2) }}</div>
      </div>
    </section>
  @elseif($postings->isNotEmpty())
    <section class="cvs27-card">
      <div class="cvs27-card-head">Posted Journal · @if($row->posting_reference && $journalUrl)<a href="{{ $journalUrl }}">{{ $row->posting_reference }}</a>@else{{ $row->posting_reference ?: $row->reversal_reference }}@endif</div>
      <div class="cvs27-post">
        ...
        @foreach($postings as $p)
          <div>@if(!empty($accountLedgerUrls[$p->account_code]))<a href="{{ $accountLedgerUrls[$p->account_code] }}">{{ $p->account_code }} · {{ $p->account_name }}</a>@else{{ $p->account_code }} · {{ $p->account_name }}@endif @if($p->entry_type==='reversal') <span class="cvs27-badge">Reversal</span>@endif</div>
  ...
  @if($row->status==='reversed')
    <div class="cvs27-card"><div class="cvs27-body" style="background:#fffafa"><strong>Reversed:</strong> @if($row->reversal_reference && $reversalJournalUrl)<a href="{{ $reversalJournalUrl }}">{{ $row->reversal_reference }}</a>@else{{ $row->reversal_reference }}@endif · {{ $row->reversal_reason }}</div></div>
  @endif
