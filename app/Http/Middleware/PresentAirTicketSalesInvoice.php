...
Array.from(document.querySelectorAll('*')).filter(el=>el.children.length===0).forEach(el=>{
    ...
    if(t.includes('sales invoice')&&t.includes('erp-09')){
        el.textContent='SALES INVOICE · ERP-11.1';
...
function metricCard(label){
    const node=Array.from(document.querySelectorAll('*')).find(el=>el.children.length===0&&norm(el.textContent)===norm(label)&&el.getBoundingClientRect().top<topBoundary);
    if(!node)return null;let current=node.parentElement;
    for(let i=0;i<6&&current;i++){const r=current.getBoundingClientRect();if(r.width>110&&r.height>50&&r.height<220)return current;current=current.parentElement;}
    return node.parentElement;
}
function setMetric(label,value,sub){
    const card=metricCard(label);if(!card)return;card.classList.add('et-si11-metric-103179');
    const leaves=Array.from(card.querySelectorAll('*')).filter(el=>el.children.length===0&&String(el.textContent||'').trim()!=='');
    const idx=leaves.findIndex(el=>norm(el.textContent)===norm(label));if(idx<0)return;
    const valueNode=leaves.slice(idx+1).find(el=>norm(el.textContent)!=='');if(valueNode)valueNode.textContent=value;
    const subNode=leaves.slice(idx+2).find(el=>norm(el.textContent)!=='');if(subNode&&sub)subNode.textContent=sub;
}
setMetric('Invoice Total',money(sourceTotal),'Total amount for this invoice');
setMetric('Passengers',String(ticketCount),String(paxMix.ADULT)+' Adult · '+String(paxMix.CHILD)+' Child · '+String(paxMix.INFANT)+' Infant');
setMetric('Service Lines',String(groups.length),'Grouped by fare type + customer rate');
const accountingMetric=metricCard('Accounting');if(accountingMetric)accountingMetric.classList.add('et-si11-metric-103179');
...
const linesHtml=groups.length?groups.map(group=>{
    ...
    return '<article class="et-si11-line-103179"><div class="et-si11-line-top-103179"><div class="et-si11-line-title-103179"><span>'+esc(group.fare_type)+'</span> · Air Ticket</div><div class="et-si11-line-amount-103179">'+money(group.total)+'</div></div><div class="et-si11-line-metrics-103179"><div class="et-si11-line-metric-103179"><small>Qty</small><strong>'+Number(group.quantity||0).toLocaleString()+'</strong></div><div class="et-si11-line-metric-103179"><small>Unit Price</small><strong>'+money(group.rate)+'</strong></div><div class="et-si11-line-metric-103179"><small>Discount</small><strong>PKR 0</strong></div></div>'+(ticketHtml?'<div class="et-si11-ticket-box-103179">'+ticketHtml+'</div>':'')+'</article>';
...
if(accountingCard)accountingCard.classList.add('et-si11-card-103179','et-si11-accounting-103179');
