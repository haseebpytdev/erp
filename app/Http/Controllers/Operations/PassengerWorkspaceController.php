...
final class PassengerWorkspaceController extends Controller
{
    ...
    public function update(Request $request, string $source, int $passenger)
    {
        ...
        $updates=[]; foreach ([['title','salutation','title'],['sex','gender','sex'],['first_name','given_name','first_name'],['last_name','surname','last_name'],['passport_no','passport_number','passport_no'],['date_of_birth','dob','date_of_birth'],['passport_expiry','passport_expiry_date','passport_expiry'],['nationality','nationality_name','nationality'],['issuing_country','passport_issuing_country','issuing_country']] as [$a,$b,$k]) { foreach([$a,$b] as $c) if(in_array($c,$columns,true)){ $updates[$c]=$data[$k]??null; break; } }
