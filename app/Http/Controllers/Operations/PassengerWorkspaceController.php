...
final class PassengerWorkspaceController extends Controller
{
    ...
    public function update(Request $request, string $source, int $passenger)
    {
        ...
        $duplicate = DB::table($table)->where('id','<>',$passenger)->where(function($q) use ($columns,$data) { foreach (['passport_no','passport_number','passport','passport_id','document_number'] as $c) if(in_array($c,$columns,true)) $q->orWhereRaw("UPPER(REPLACE(`$c`, ' ', '')) = ?", [$data['passport_no']]); })->exists();
