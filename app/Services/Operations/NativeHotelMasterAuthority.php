...
final class NativeHotelMasterAuthority
{
    ...
    {
        $tables=$this->tables(); if(!$tables['hotel']) throw ValidationException::withMessages(['hotel'=>'Native Hotel Master could not be resolved.']); $unresolved=$this->requiredUnresolvedColumns($tables['hotel']);
        $cities=$this->cityRows($tables['city']); $hotels=$this->hotelRows($tables['hotel']); if($unresolved){$rows=array_map(fn($r)=>$r+['status'=>'INVALID','reason'=>'Required columns unresolved: '.implode(', ',$unresolved),'required_unresolved_columns'=>$unresolved],$rows); return ['rows'=>$rows,'summary'=>['total'=>count($rows),'new'=>0,'existing'=>0,'alias_matches'=>0,'invalid'=>count($rows),'required_unresolved_columns'=>$unresolved]];} $out=[]; $seen=[];
    ...
    {
        $preview=$this->preview($rows); $tables=$this->tables(); $unresolved=$this->requiredUnresolvedColumns($tables['hotel']); if($unresolved) throw ValidationException::withMessages(['hotel'=>'Required columns unresolved: '.implode(', ',$unresolved)]); $columns=Schema::getColumnListing($tables['hotel']); $cityColumns=$tables['city']?Schema::getColumnListing($tables['city']):[]; $created=0;
    ...
    private function identity(array $city,string $name):string{return ((int)($city['id']??0)>0)?'FK:'.$this->key((int)$city['id'],$name):'TEXT:'.$this->canonicalCityFamily((string)($city['name']??'')).'|'.$this->canonicalHotelName($name);}
