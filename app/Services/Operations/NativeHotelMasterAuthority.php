...
final class NativeHotelMasterAuthority
{
    ...
    public function import(array $rows): array
    {
        ...
        DB::transaction(function() use (&$created,$preview,$tables,$columns,$cityColumns){$cities=$this->cityRows($tables['city']);$hotels=$this->hotelRows($tables['hotel']);foreach($preview['rows'] as $r){if(($r['status']??'')!=='NEW')continue;$row=[];$this->put($row,$columns,['name','hotel_name','title','property_name'],$r['hotel_name']);$this->put($row,$columns,['city_id','travel_city_id'],$r['city_id']);$this->put($row,$columns,['city_iata','iata','iata_code','city_code'],$this->cityCode($r['resolved_city']));$this->put($row,$columns,['country','country_code','country_iso'],'SA');$this->put($row,$columns,['code','hotel_code','property_code'],$r['generated_code']);$this->put($row,$columns,['is_active','active'],1);$row=array_intersect_key($row,array_flip($columns));if(!$row)throw ValidationException::withMessages(['hotel'=>'Native Hotel fields could not be resolved.']);DB::table($tables['hotel'])->insert($row);$created++;}});
