...
var etgpAirSearchableAirline113330=function(label,value,airlines,legacyValue){
  ...
  var close=function(){popup.hidden=true;input.setAttribute('aria-expanded','false');input.removeAttribute('data-etgp-airline-active');if(airlines.length&&selected===null&&String(input.value||'').trim()){if(lastValid){input.value=labelFor(lastValid);selected=lastValid;}else{input.value='';input.setAttribute('data-etgp-airline-error','Select an airline from the list.');}}};
  ...
  if(document&&document.addEventListener)document.addEventListener('click',function(event){var target=event.target;if(target!==input&&(!unit.contains||!unit.contains(target)))close();});
...
var renderPageItinerary113324=function(container,pageState,data,rerender){
  pageState.segments=etgpAirNormalizeBlankSegments113330(pageState.segments);if(!pageState.segments.length)pageState.segments=[{id:0,client_key:'segment-new-'+Date.now()+'-1',segment_type:'outbound',from:'',to:'',airline:'',airline_code:'',flight_number:'',departure_at:null,arrival_at:null}];
